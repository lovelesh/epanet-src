...
#define myMAXITER 30
#define myOUTER_MAXITER 2000000
#define DEBUG 1
...
int main(int argc, char *argv[])
{
  ...
  ENReadOutFlow(f_demand_input, tankcontrol, timeperiod);   //Read tank demands (outflow) from file. Need to see if it can be integrated with Epanet.
   ...
   // Initialise Valve Controls
	...
	printf(" Initial Valvue Value = %f",valvecontrol[temp_count].ValveValues[0]);
      }
    }
...
{
  /*
   * Reads the intial valve setting for times 0 to timeperiod-1.
  ...
  */
    ...
    while ((fgets(line, 1024, stream)!=NULL) && (valvecount < Nvalves))
    {
        ...
        tmp = strdup(line);
	tok = strtok(tmp, ",");
	if(tok !=NULL) {strcpy(valvecontrol[valvecount].ValveID,tok);}
...
//     
	...
	func_value += 24* (tankcontrol_current[temp_count2].TankLevels[0]-(tankcontrol_current[temp_count2].TankLevels[timeperiod] + temp_tank_level_difference)) * (tankcontrol_current[temp_count2].TankLevels[0]-(tankcontrol_current[temp_count2].TankLevels[timeperiod] + temp_tank_level_difference)) /(tankcontrol_current[temp_count2].MaxTankLevel); // penalise non periodicity.
...
//       temp_float_var_min = tankcontrol_current[temp_count2].TankLevels[temp_count]-tankcontrol_current[temp_count2].MinTankLevel;
      func_value+=pow(temp_float_var_max,4)/pow((tankcontrol_current[temp_count2].MaxTankLevel),3);      
   ...
   for(temp_count = 0 ; temp_count <Nvalves; temp_count++)
   {
     for(temp_count2 = 0; temp_count2 < timeperiod; temp_count2++)
     {      
      temp_float_var_max = (valvecontrol_current[temp_count].ValveValues[temp_count2]-MAX_VALVEVALUE)/MAX_VALVEVALUE;
      if(temp_float_var_max > 0) {func_value+=temp_float_var_max*temp_float_var_max;}  

      func_value+=valvecontrol_current[temp_count].ValveValues[temp_count2]/(MAX_VALVEVALUE);
     }
   }
...
void update_control(struct TankStruct *tankcontrol_current, struct ValveStruct *valvecontrol_current,struct TankStruct *tankcontrol_gradient,struct ValveStruct *valvecontrol_gradient, int main_iteration_count)
{
 ...
 if(((main_iteration_count%PRINT_INTERVAL_FUNCTIONVALUE)==0)) {printf(" Gradient Norm = %f alpha mult = %f",norm_grad_var,alpha_mult);} 
