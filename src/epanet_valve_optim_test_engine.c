...
unsigned long Job_Handler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol);
...
void compute_flows(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol, unsigned long simulation_time);
...
int main(int argc, char *argv[])
{
	...
	printf("\n==============================================================================================\n");
	printf("                                  EPANET STARTED SUCESSFULLY                               \n");
	printf("==============================================================================================\n");
	...
	ENReadOutFlow(f_demand_input, tankcontrol, timeperiod);   //Read tank demands (outflow) from file.
	...
	/*
	while(iteration_count <= myMAXITER){
		// Schedule the jobs
		...
		display_output(tankcontrol, valvecontrol, timeperiod);
		//iteration_count++;
	}
	*/	
...
void ENReadOutFlow(char *f_demand_input, struct TankStruct *tankcontrol, int timeperiod)
{
	 ...
	 * f_demand_input 	: file name of file containing outflow demand data. 
	 * Each tank data should be in one line, and each field separated by a comma.
	 * First field should be the tank ID.
...
void Job_Scheduler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	for(temp_count2 = 0; temp_count2 < Nvalves; temp_count2++){
		for(temp_count = 0; temp_count < valvecontrol[temp_count2].TimePeriod; temp_count = temp_count + hourly_scheduler){
			...
			p.hours = temp_count;
			p.minutes = 0;
			strcpy(p.id, valvecontrol[temp_count2].ValveID);
			p.value = valvecontrol[temp_count2].ValveValues[temp_count];
			Qpush(p.keyword, p.hours, p.minutes, p.id, p.value);
		}
	}
	// Check the queue
...
void compute_flows(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol, unsigned long simulation_time)
{
	...
	for(temp_count=0; temp_count<Ntanks; temp_count++) {
		ENgetlinkvalue(tankcontrol[temp_count].InputLink,EN_FLOW, &(tankcontrol[temp_count].InFlow[simulation_time]));
		ENgetlinkvalue(tankcontrol[temp_count].OutputLink,EN_FLOW, &(tankcontrol[temp_count].OutFlow_Epanet[simulation_time]));
	}
...
void update_tank_level(struct TankStruct *tankcontrol)
{
	...
	for(temp_count = 0 ; temp_count < timeperiod; temp_count++) {
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2++) {
			tankcontrol[temp_count2].TankLevels[temp_count+1] = 
			tankcontrol[temp_count2].TankLevels[temp_count]+
			(tankcontrol[temp_count2].InFlow[temp_count]-tankcontrol[temp_count2].OutFlow[temp_count]-tankcontrol[temp_count2].OutFlow_Epanet[temp_count])
			*LPS_TANKUNITS; //TankLevel is water volume in meter^3.
		}
	}
}
	...
	for(temp_count = 0; temp_count< Nvalves; temp_count++){
		...
		for(temp_count2 = 0; temp_count2 < timeperiod; temp_count2++){
			printf("\n Valve Value [%d] = %f \t Gradient = %f",
				   temp_count2,valvecontrol[temp_count].ValveValues[temp_count2],
...
void ENOptimiseValve(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	 ...
	 * Important part of Minimiser.
	 * Calculates the objective function of the current setting and suggests changes
	...
	memcpy(tankcontrol_current,tankcontrol,Ntanks*sizeof(struct TankStruct)); //Dont know if this will work.
	...
	// Calculating the objective_function for previous setting
...
{
	/*
	 *Calculates the objective value for a particular tank and valve setting
	 * Three penalty functions
	 * 1) |Th(t,h) - Mid(t)|^2    //Distance from minpoint of the tank
	 * 2) |Th(t,h) - Th(t,24)|^2  //Periodicity of the tanks
	 * 3) |V(t) - V(max)|^2       //Distance from max valve setting
	 * 
	*/
...
void update_control(struct TankStruct *tankcontrol_current, struct ValveStruct *valvecontrol_current,struct TankStruct *tankcontrol_gradient,struct ValveStruct *valvecontrol_gradient, int main_iteration_count)
{
	...
	while(((function_value_next-function_value_current)>= -c *alpha_mult* norm_grad_var) && (iteration_count < myMAXITER)) {
		...
		for(temp_count =0; temp_count <Ntanks; temp_count++) {
			// random_direction(&random_number,1);
			temp_tankstruct[temp_count].TankLevels[0] = tankcontrol_current[temp_count].TankLevels[0]-
			(tankcontrol_gradient[temp_count].TankLevels[0])*alpha_mult;
		}
	...
	for(temp_count2=0; temp_count2<timeperiod; temp_count2++) {
		for(temp_count =0; temp_count <Nvalves; temp_count++) {
			random_direction(&random_number,1);
			temp_valve_value = valvecontrol_current[temp_count].ValveValues[temp_count2]-
			valvecontrol_gradient[temp_count].ValveValues[temp_count2]*
			(1+random_number*pow((main_iteration_count/100)+1,-0.6))*alpha_mult;
	...
	for(temp_count =0; temp_count <Ntanks; temp_count++) {
		random_direction(&random_number,1);
		temp_tankstruct[temp_count].TankLevels[0] = tankcontrol_current[temp_count].TankLevels[0]-
		(tankcontrol_gradient[temp_count].TankLevels[0])*
		(1+random_number*pow((main_iteration_count/100)+1,-beta))*alpha_mult;	}
